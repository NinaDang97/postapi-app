...
export function createPost(newPost) {
  const request = axios.post(`${ROOT_URL}/api/posts`, newPost);
  return {
    type: CREATE_POST,
    payload: request
  };
...
export function fetchPost(postId) {
  const request = axios.get(`${ROOT_URL}/api/posts/${postId}`);
  return {
    type: FETCH_POST,
    payload: request
  };
...
export function deletePost(postId) {
  const request = axios.delete(`${ROOT_URL}/api/posts/${postId}`);
  return {
    type: DELETE_POST,
    payload: request
  };
